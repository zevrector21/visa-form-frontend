...
export const ApiManager = {
  ...
  DeleteDS160Application: (applicationId, site = 'default') => requests.put(`ds-160/archive/${applicationId}`, {}, site),
  // DeleteDS160Application: (applicationId, site = 'default') => requests.del(`ds-160/${applicationId}`, site),
  // DeleteCaEtaApplication: (applicationId, site = 'canada') => requests.del(`ds-160/${applicationId}`, site),
  DeleteCaEtaApplication: (applicationId, site = 'canada') => requests.put(`ds-160/archive/${applicationId}`, {}, site),
  ...
  GetPaymentTemplatesList: (headers, options, site = 'default') => requests.get(`payment?limit=${options.limit}&skip=${options.skip}`, headers, site),
  CreatePaymentTemplate: (headers, data, site = 'default') => requests.post('payment', headers, data, site),
  DeletePaymentTemplate: (country, site = 'default') => requests.del(`payment/${country}`, site),
  UpdatePaymentTemplate: (payment, site = 'default') => requests.put(`payment/${payment.country}`, payment, site),
