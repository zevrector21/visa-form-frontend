    ...
    return (
      <Form {...formItemLayout}>
        ...
        <div className="visa-form-bottom-btn-group">
          ...
          {showPrev && <Button id="Prev" style={{ marginRight: 8 }} onClick={e => handlePrev(e, form, this.handleDates)}>{tr(resources.prev)}</Button>}
          {showNext && <Button type="primary" onClick={e => handleNext(e, form, this.handleDates)}>{tr(resources.next)}</Button>}
          <Button type="link" onClick={e => handleSave(e, form, this.handleDates)}>{tr(resources.save_and_continue_later)}</Button>
        </div>
      </Form>
